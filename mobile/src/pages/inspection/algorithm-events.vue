<template>
  <view class="page">
    ...
    <view v-for="event in events" :key="event.id" class="card">
      ...
      <view v-if="event.state === 'OPEN'" class="reviews">
        <button size="mini" @click="review(event.id, 'CONFIRMED')">确认异常</button>
        <button size="mini" @click="review(event.id, 'FALSE_POSITIVE')">标记误报</button>
      </view>
    </view>
  </view>
</template>
...
import { createEnvironmentAlgorithms, createPersonnelAlgorithms, queryAlgorithmEvents, queryAlgorithms, recoverAlgorithmEvent, reviewAlgorithmEvent, type AlgorithmDefinition, type AlgorithmEvent } from '@/api/inspection'
...
async function review(id: number, status: 'CONFIRMED' | 'FALSE_POSITIVE') { await reviewAlgorithmEvent(id, status); await load() }
