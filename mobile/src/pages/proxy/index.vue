<template>
  <view class="page">
    <PageScroller :loading="loading" @refresh="reload" @load-more="load()">
      <SearchBar v-model="query" placeholder="代理名称、应用或流 ID" @search="reload" />
      <view class="list">
        <view v-for="item in items" :key="item.id" class="card">
          <view class="card__top">
            <text class="card__title">{{ item.name || item.stream || '未命名代理' }}</text>
            <StatusBadge :value="item.pulling ?? item.status" />
          </view>
          <text class="card__meta">应用：{{ item.app || '-' }}\n流 ID：{{ item.stream || '-' }}</text>
        </view>
        <EmptyState v-if="!loading && !items.length" title="暂无拉流代理" />
...
import { queryProxies, type StreamItem } from '@/api/stream'
