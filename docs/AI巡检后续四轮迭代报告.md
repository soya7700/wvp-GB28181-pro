...
- 统一记录在线、取流、首帧耗时、视频质量和录像完整性。
- 自动计算 0～100 健康分，并分为健康、关注、严重。
- 移动端提供整体健康分和问题点位列表。
...
- AI 异常确认后自动生成唯一工单。
- 支持接单、提交解决、复检通过或退回。
- 根据优先级计算 2、8、24 小时 SLA 截止时间。
- 移动端覆盖待接单、处理中、待复检和已关闭状态。

### 第七轮：事件聚合与根因

- 同设备同根因五分钟内自动聚合，抑制重复事件。
- 根因分为设备/网络、媒体/网络、存储、AI 服务、摄像机/场景。
- 支持聚合事件确认恢复并关闭相关异常。
...
- 识别结果关联模型、规则。
- 统计确认率、误报率和有效样本量。
- 样本少于 20 标记样本不足，误报率超过 30% 标记漂移风险。
- 支持 0～100% 灰度流量和模型全量激活。
...
1. `AI巡检-迭代4-*`：视频健康表。
2. `AI巡检-迭代5-*`：巡检工单表。
3. `AI巡检-迭代6-*`：事件聚合与根因字段。
4. `AI巡检-迭代7-*`：模型关联和灰度字段。

`*` 根据数据库选择 `mysql` 或 `postgresql-kingbase`。
